...
老男人的青春 2019-07-30 18:58:54  75  收藏
...
1、 从Master拉取创建新分支Feature进行开发，开发完成合并到Develop发测试。

2、 测试期间Bug都在Feature修改，之后合并到Develop。

3、 测试通过后，合并Feature到Master，发布Master到生产。

4、 发布期间Bug都在Feature修改，之后合并Feature到Develop和Master，并在Master上打Tag记录版本号/任务编号

二、 Bug修复：

1、 从Master拉取创建新分支Hotfix进行修复，修复完毕后合并到Develop发测试。

2、 测试期间Bug都在Hotfix修改，之后合并到Develop。

3、 测试通过后，合并Hotfix到Master，并打Tag记录缺陷编号，发布Master到生产。

三、 commit：

 1、迭代开发功能：版本+开发内容，如：1.0.0：会议管理开发、部分样式优化。

 2、线上Bug修复：fixbug+修改内容，如：fixbug：会议管理导出bug

备注：

1、测试服务器对应分支develop、生产服务器对应分支master、维护好自己本地分支。

2、发生产除特殊情况下，均在晚上发布。

3、发布测试或者生产均不可删除服务器上旧文件。
